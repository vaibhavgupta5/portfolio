...
import { IconHome, IconUser,  IconAddressBook, IconBrandGithub } from "@tabler/icons-react";
...
export default function Home() {
  ...
  const navItems = [
    { title: "Home", icon: <IconHome className="h-5 w-5" />, onClick: () => setcurrPage("home") },
    { title: "Resume", icon: <IconUser className="h-5 w-5" />, onClick: () => setcurrPage("resume") },
    { title: "Projects", icon: <IconBrandGithub className="h-5 w-5" />, onClick: () => setcurrPage("projects") },
    { title: "Contact", icon: <IconAddressBook className="h-5 w-5" />, onClick: () => setcurrPage("contact") },
  ];
  ...
  return (
    <div className="flex  justify-center relative flex-col rounded-2xl border-solid border-[1px] border-[#3C3C3C] bg-[#1E1E1F] ">
      ...
      <FloatingDock items={navItems} desktopClassName="fixed bottom-4 left-1/2 -translate-x-1/2" mobileClassName="fixed bottom-4 right-4 z-50" />
